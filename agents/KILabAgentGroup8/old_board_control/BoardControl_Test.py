    ...
    for snake in board.snakes:
        ...
        for i in range(len(snake.body)):
            if snake.snake_id == you_snake_id:
                ...
                snake_you[i + 1] = snake.body[-i-1]
            else:
                state[snake.body[-i-1].x][snake.body[-i-1].y] = i + 1
                snake_opp[i + 1] = snake.body[-i-1]
    ...
    if length_you >= length_opp:
        for i in range(1, 121):
            if not queue and not queue_opp:
                break
            if i in snake_you:
                if Control_state[snake_you[i].x][snake_you[i].y] == 200:
                    Control_state[snake_you[i].x][snake_you[i].y] = 0
            if i in snake_opp:
                if Control_state[snake_opp[i].x][snake_opp[i].y] == 200:
                    Control_state[snake_opp[i].x][snake_opp[i].y] = 0
            if not queue:
                if low_wall_you > Control_you + 1:
                    you_end = False
            if not queue_opp:
                if low_wall_opp > Control_opp + 1:
                    opp_end = False
            while queue:
