...
def BoardControl(board: BoardState, you: Snake):
    ...
    if len(board.snakes) == 0:
        for snake in board.dead_snakes:
            if snake.snake_id == you_snake_id:
                ...
                length_opp[snake.snake_id] = len(snake.body) 

        if all(length_you > value for value in length_opp.values()):
            return 121 # win
        if all(length_you < value for value in length_opp.values()):
            return -121 # loss
        return 0 # tie
    ...
    for snake in board.snakes:
        ...
        if snake.snake_id == you_snake_id:
            ...
            Control_state[snake.body[0].x][snake.body[0].y] = -1 # mark snake head in control state -1
            length_opp[snake.snake_id] = len(snake.body) # NOTE where to store length of opp
    ...
    if all(length_you > value for value in length_opp.values()):
        ...
        # 11*11 = 121
        for i in range(1, board.height*board.width):
            ...
            for snake in board.snakes:
                if snake.snake_id != you_snake_id:
                    ...
                    opp_end = almost_dead(board, queue_opp, length_opp[opp_snake_id], Control_opp, opp_food, low_wall_opp, opp_end, opp_snake_id, Control_state, state)
                    if opp_end:
                        break
        ...
        for i in range(1, board.height*board.width):
            ...
            for snake in board.snakes:
                if snake.snake_id != you_snake_id:
                    opp_snake_id = snake.snake_id
                    opp_end = almost_dead(board, queue_opp, length_opp[snake.snake_id], Control_opp, opp_food, low_wall_opp, opp_end, opp_snake_id, Control_state, state)
                    if opp_end:
                        break
    ...
    result = (Control_you - Control_opp) / 2
    if opp_end != you_end:
