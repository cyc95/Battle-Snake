...
class KILabAgentMCTS(BaseAgent):

    def get_name(self):
        return 'Gruppe8+BoardControlV1.6'
    ...
    def move(self, game_info: GameInfo, turn: int, board: BoardState, you: Snake) -> MoveResult:
        ...
        clib = cdll.LoadLibrary('./libuntitled2.so')
        ...
        simulation_time = 300
        action = clib.getaction(array_state, you_snake_head, opp_snake_head, you_snake_length, opp_snake_length, you_snake_health, opp_snake_health, you_snake_direction, opp_snake_direction, simulation_time)
        if action == 0:
            action = Direction.UP
        if action == 1:
