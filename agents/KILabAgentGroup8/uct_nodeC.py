...
class UCTNode:
    ...
    def policy_snake_test(self):
        ...
        value = np.zeros(3)
        for i in range(3):
            if action_value[i*3] >= 121 and action_value[i*3+1] >= 121 and action_value[i*3+2] >= 121:
                value[i] = (action_value[i*3] + action_value[i*3+1] + action_value[i*3+2]) / 3
                return value
        for i in range(3):
            if action_value[i] <= -121 and action_value[i+3] <= -121 and action_value[i+6] <= -121:
    ...
    def __eq__(self, other):
        order = ['state', 'active_player', 'action', 'children', 'num_actions', 'valid_actions', 'child_values', 'child_visits', 'parent']
    ...
    def uct(self, c: float) -> np.ndarray:
        ...
        for i in range(9):
            if not self.child_valid[i]:
                ...
                if self.child_visits[i] == 0:
                    uct[i] = np.inf
                else:
                    uct[i] = self.child_Average_values[i] / 121 + c * np.sqrt(ln_n/self.child_visits[i])
            if i in self.children:
                if self.children[i].end:
                    uct[i] = -np.inf
        return uct
    ...
    def expand(self, env, next_action: int):
        ...
        opp_snake = copy.deepcopy([n for n in self.state.get_alive_and_dead_snakes() if n.snake_id != self.active_player][0])
        ...
        env.step_action(snack.possible_actions()[int(next_action / 3)], opp_snake.possible_actions()[int(next_action % 3)], self.active_player)
        child = UCTNode(
    ...
    def backup(self, winner):
        ...
        while current_node.parent is not None:
            ...
            current_node.parent.child_Average_values[current_node.action] = current_node.parent.child_values[current_node.action] / current_node.parent.child_visits[current_node.action]
