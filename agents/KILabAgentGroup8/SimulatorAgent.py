...
class SimulatorAgent(BaseAgent):
    ...
    def survive(slef, you: Snake,
        ...
        while not queue.empty():
            ...
            valid_neighbors = [Position(x=n[0], y=n[1]) for n in neighbors if
                               grid_map.is_valid_at(n[0], n[1]) and not board.is_occupied_by_snake(
                                   Position(n[0], n[1]))]
            ...
            for neighbor in valid_neighbors:
                if neighbor not in cost_so_far:
                    ...
                    number_of_valid_neighbors = len([Position(x=n[0], y=n[1]) for n in neighbors if
                                                     grid_map.is_valid_at(n[0],
                                                                          n[1]) and not board.is_occupied_by_snake(
                                                         Position(n[0], n[1]))])
    ...
    def follow_food(self, snake: Snake, board: BoardState, grid_map: GridMap):
        ...
        for food in board.food:
            distance, path = self.a_star_search(head, food, board, grid_map)
            if distance < lowcost:
                bestpath = path
                lowcost = distance
        return bestpath
    ...
    def find_position(self, grid_map, policy):
        if policy[0] == 'x':
            ...
            while grid_map.grid_cache[bestposition.x][bestposition.y] == Occupant.Snake or \
                    grid_map.grid_cache[bestposition.x][
                        int(bestposition.y - (bestposition.y - 5) / abs(bestposition.y - 5))] == Occupant.Snake:
                if policy[1] <= 5:
                    bestposition.x += 1
        ...
        if policy[0] == 'y':
            ...
            while grid_map.grid_cache[bestposition.x][bestposition.y] == Occupant.Snake or \
                    grid_map.grid_cache[int(bestposition.x - (bestposition.x - 5) / abs(bestposition.x - 5))][
                        bestposition.y] == Occupant.Snake:
                if policy[1] <= 5:
                    bestposition.y += 1
                    if bestposition.y > 10:
                        bestposition.y = 0
    ...
    def a_star_search(start_field: Position,
        ...
        while not queue.empty():
            ...
            if current_field.is_position_equal_to(Ziel):
                break
            # find all neighbors that are valid
            neighbors = [
                [current_field.x + 1, current_field.y],
