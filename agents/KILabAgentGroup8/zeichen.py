...
def BoardControl(board: BoardState, you: Snake):
    ...
    if opp_end != you_end:
        if not you_end:
            result = Control_you / 2 - 120
        else:
            result = 120 - Control_opp / 2
    zeichen_state = np.zeros(shape=(board.width + 1, board.height + 1), dtype='int32')
    zeichen_Control_state = np.zeros(shape=(board.width + 1, board.height + 1), dtype='int32')
    for i in range(11):
        for j in range(11):
            zeichen_state[i][j] = state[j][10-i]
            if zeichen_state[i][j] == 500:
                zeichen_state[i][j] = 0
        zeichen_state[i][11] = 10
        zeichen_state[11][i] = 10
    for i in range(11):
        for j in range(11):
            zeichen_Control_state[i][j] = Control_state[j][10-i]
        zeichen_Control_state[i][11] = 10
        zeichen_Control_state[11][i] = 10

    for i in zeichen_state:
        print(i)
    for i in zeichen_Control_state:
        print(i)
    print(result)
    return result
