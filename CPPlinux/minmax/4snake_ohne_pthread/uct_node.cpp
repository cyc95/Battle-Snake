...
void cyc::expand(simulation_environment* env, cyc::uct_node *current_node){
    if(!current_node->expanded){
        ...
        int i,j,t,d;
        int opp_action[3], opp_action_num[3];
        int you_possible_action[3], opp_possible_action[3],opp_possible_action2[3],opp_possible_action3[3];
        ...
        get_possible_actions(&opp_possible_action3[0], current_node->opp_snake_direction[2]);
        for(i=0;i<3;i++){
            for(j=0;j<3;j++){
                for(t=0;t<3;t++) {
                    for (d=0;d<3;d++) {
                        ...
                        opp_action[2] = opp_possible_action3[d];
                        opp_action_num[0] = j;
                        opp_action_num[1] = t;
                        opp_action_num[2] = d;
                        ...
                        if (env->step(you_possible_action[i], opp_possible_action[j],opp_possible_action2[t],opp_possible_action3[d])) {
                            // game over
                            ...
                            current_node->child_values[i][j][t][d][0] = *result;
                            current_node->child_values[i][j][t][d][1] = *(result+1);
                            current_node->child_values[i][j][t][d][2] = *(result+2);
                            current_node->child_values[i][j][t][d][3] = *(result+3);
                            current_node->child_valid[i][j][t][d] = false;
                            current_node->child_visits[i][j][t][d] = 1;
                        } else {
                            // game not over
                            cyc::board_control(result, &env->state[0][0], env->you_snake_head,
                                               env->opp_snake_head, env->you_snake_length,
                            ...
                            current_node->children[i][j][t][d] = p;
