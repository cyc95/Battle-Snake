...
extern "C" int getaction(int array_state[121], int you_snake_head[2], int opp_snake_head[6], int you_snake_length, int opp_snake_length[3], int you_snake_health, int opp_snake_health[3], int you_snake_direction, int opp_snake_direction[3], int simulation_time){
    ...
    for(i=0;i<3;i++){ //min
        for(j=0;j<3;j++){ //min
            for(t=0;t<3;t++) {
                if (root_node->child_values[i][j][values_max_num2[i][j]][2] < root_node->child_values[i][j][t][2]) values_max_num2[i][j] = t;
            }
        }
    }
    for(i=0;i<3;i++){ // min
        for(j=0;j<3;j++) { // min
            if (root_node->child_values[i][values_max_num[i]][values_max_num2[i][values_max_num[i]]][1] < root_node->child_values[i][j][values_max_num2[i][j]][1]) values_max_num[i] = j;
            //   this->child_values[i][j]                [values_max_num2[i][j                ]][values_max_num3[i][j]                [values_max_num2[i][j                ]]][1]
        }
    }

    for(i=0;i<3;i++){ //max
        test[i] = root_node->child_values[i][values_max_num[i]][values_max_num2[i][values_max_num[i]]][0];
    }
    for(i=0;i<3;i++){ //max
    ...
    if(!action_A_search[max]) {
        ...
        if(you_snake_health > 80  && you_snake_length < max_opp_length + 4  && test[action_a] > 30  && test[max] < 80 && action_A_search[action_a]) max = action_a;
        if(you_snake_health <= 80 && test[action_a] > 25  && test[max] < 80  && action_A_search[action_a]) max = action_a;
        if(you_snake_health <= 60 && test[action_a] > 18  && action_A_search[action_a]) max = action_a;
        if(you_snake_health <= 30 && test[action_a] > 13  && action_A_search[action_a]) max = action_a;
        if(you_snake_health <= 10 && test[action_a] > 10  && action_A_search[action_a]) max = action_a;
    }
    delete env;
    delete root_node;
    // return kkk;
