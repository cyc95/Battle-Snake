...
void vector_initialization(const int* input_you_snake_head, const int* input_opp_snake_head, std::vector<cyc::position> &you_vector, std::vector<cyc::position> &opp_vector, std::vector<cyc::position> &opp_vector2, const int* input_opp_snake_health);
void calculate_field(float* result,std::vector<cyc::position> &you_vector, std::vector<cyc::position> &opp_vector, std::vector<cyc::position> &opp_vector2, const int* input_state, int* state, int* control_state, const int &input_you_snake_length, const int *input_opp_snake_length, const int &input_you_snake_health,  const int *input_opp_snake_health, const int *input_you_snake_head, const int * input_opp_snake_head);
        ...
        if (!*input_opp_snake_health && !*(input_opp_snake_health + 1) && !*(input_opp_snake_health + 2)) {
            ...
            *(result+2) = 0;
            ...
            vector_initialization(input_you_snake_head, input_opp_snake_head, you_vector, opp_vector, opp_vector2,
                                   input_opp_snake_health);
            calculate_field(result, you_vector, opp_vector, opp_vector2, input_state, state, control_state,
...
void find_not_checked_position(int* control_state, const int* input_state, float &control_you, float &control_opp,float &control_opp2, std::vector<cyc::position> &you_vector, std::vector<cyc::position> &opp_vector, std::vector<cyc::position> &opp_vector2){
    int i,j,w;
    bool flag_you, flag_opp,flag_opp2;
    int valid_neighbors[4][2];
    for(i=0;i<121;i++){
    ...
    for(i=0;i<11;i++){
        for(j=0;j<11;j++){
            if(*(control_state+i*11+j)==0){
                ...
                if(flag_you && !flag_opp && !flag_opp2 ){
...
void calculate_field(float* result, std::vector<cyc::position> &you_vector, std::vector<cyc::position> &opp_vector, std::vector<cyc::position> &opp_vector2,  const int* input_state, int* state, int* control_state, const int &input_you_snake_length, const int *input_opp_snake_length, const int &input_you_snake_health,  const int *input_opp_snake_health, const int *input_you_snake_head, const int * input_opp_snake_head){
    float control_you=0, control_opp=0, control_opp2=0;
    ...
    if(input_you_snake_length>opp_snake_length_max){
        for (i = 0; i < 121; i++) {
            if (you_vector.empty() && opp_vector.empty() && opp_vector2.empty()) break;
            ...
            check_neighbors(opp_vector2, opp_vector_new2, opp_food2, control_opp2, control_state, state, low_wall_opp2,
                            i, -2);
            sort(you_vector, you_vector_new, opp_vector, opp_vector_new, opp_vector2, opp_vector_new2);
        }
    }
    else {
        for (i = 0; i < 121; i++) {
    ...
    find_not_checked_position(control_state, input_state, control_you, control_opp, control_opp2, you_vector, opp_vector, opp_vector2);
    for(i=0;i<121;i++){
    ...
    if(!you_alive) {
        ...
        if(!opp_alive){
            control_opp = old_control_opp / 3;
            control_you = control_you + control_opp;
            control_opp2 = control_opp2 + control_opp;
        }
        if(!opp_alive2){
            control_opp2 =control_opp2 -  3 * old_control_opp2 / 3;
            control_you = control_you + old_control_opp2 / 3;
            control_opp = control_opp + old_control_opp2 / 3;
        }
