...
namespace cyc {
    ...
    public:      //There are no private variables. The variables are directly used for speed //health -> 0 = death
        ...
        uct_node(const int* input_state, const int* input_you_snake_head, const int* input_opp_snake_head, const int &input_you_snake_length, const int* input_opp_snake_length, const int &input_you_snake_health, const int* input_opp_snake_health, const int &input_you_snake_direction, const int* input_opp_snake_direction, const int &input_you_snake_direction_num, const int *input_opp_snake_direction_num) {
            ...
            opp_snake_direction[0] = *opp_snake_direction;
            opp_snake_direction[1] = *(opp_snake_direction + 1);
            opp_snake_direction[2] = *(opp_snake_direction + 2);
...
*/
        ...
        void backup(){
            ...
            while(current_node->parent != nullptr){
                ...
                if(val>0) val -= 0.005;
                if(val<0) val += 0.005;
                current_node->parent->child_values[current_node->you_snake_direction_num][*current_node->opp_snake_direction_num][*(current_node->opp_snake_direction_num+1)][*(current_node->opp_snake_direction_num+2)] = val;
                /*   for(i=0;i<3;i++) {
                       for (j = 0; j < 3; j++) {
                           if(current_node->child_valid[i][j]){
                               valid = true;
                ...
                if(val < -119|| val > 119) current_node->parent->child_valid[current_node->you_snake_direction_num][*current_node->opp_snake_direction_num][*(current_node->opp_snake_direction_num+1)][*(current_node->opp_snake_direction_num+2)] = false;
            ...
            if(1){//min max
                ...
                for(i=0;i<3;i++){ //min
                    for(j=0;j<3;j++){ //min
                        for(t=0;t<3;t++) {
                            for (d = 0; d < 3; d++) {
                                if (values_min[i][j] > this->child_values[i][j][t][d]) values_min[i][j] = this->child_values[i][j][t][d];
                            }
                        }
                    }
                }
                for(i=0;i<3;i++){ // min
                    for(j=0;j<3;j++) { // min
                        if(test[i] > values_min[i][j]) test[i] = values_min[i][j];
                    }
                }

                for(i=0;i<3;i++){ //max
                    if(values  < test[i])  values  = test[i];
                }
            }
            return values;
        }
