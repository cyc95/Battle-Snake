            ...
            while(current_node->parent != nullptr){
             ...
             /*   for(i=0;i<3;i++) {
                ...
                *//*
                ...
                if(val < -119|| val > 119) current_node->parent->child_valid[current_node->you_snake_direction_num][current_node->opp_snake_direction_num] = false;
...
*/
        ...
        void a_b_search(uct_node* current_node){
            int i,j,min[3]={0,0,0},max=0;
            for(i=0;i<3;i++){ //min
                for(j=0;j<3;j++){ //min
                    if(current_node->child_values[i][min[i]] > this->child_values[i][j]) min[i] = j;
                }
            }
            for(i=0;i<3;i++){ //max
                if(current_node->child_values[i][min[i]] > this->child_values[max][min[max]]) max = i;
            }
            for(i=0;i<3;i++){ //min
                for(j=0;j<3;j++){ //min
                    if( i != max && j != min[i]) current_node->child_valid[i][j] = false;
                }
            }
        }
    ...
    void expand(simulation_environment* env, uct_node* current_node);
}
