        ...
        for(i=0;i<3;i++){
            for(j=0;j<3;j++){
                ...
                if(env->step(you_possible_action[i], opp_possible_action[j])){
                    // game over
                    current_node->child_values[i][j] = board_control(&env->state[0][0], env->you_snake_head, env->opp_snake_head, env->you_snake_length, env->opp_snake_length, env->you_snake_health, env->opp_snake_health);
